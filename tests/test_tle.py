...
def test_get(run):
    ...
    r = run("space tle get name ISS")
    assert not r.stderr
    assert r.stdout == tle1
    assert r.success
    ...
    r = run("space tle get name UNKNOWN")
    assert not r.success
    assert r.stderr == "No TLE for name = 'UNKNOWN'\n"
    assert not r.stdout
...
def test_history(run):
    ...
    r = run("space tle history name ISS")
    assert not r.stderr
    assert len(r.stdout.splitlines()) == 8
    assert r.success

    r = run("space tle history name UNKNOWN")
    assert r.stderr == "No TLE for name = 'UNKNOWN'\n"
    assert not r.stdout
    assert not r.success
